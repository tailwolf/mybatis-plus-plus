...
//    }
    ...
    private static <T> T createInstanceByClass(Class clazz){
        ...
        try {
            ...
            throw new MybatisCompleteRuntimeException("无法实例化类" + clazz.getName());
        } catch (IllegalAccessException e) {
            throw new MybatisCompleteRuntimeException("无法访问实体类" + clazz.getName() + "无参构造方法！");
        } catch (InvocationTargetException e) {
            throw new MybatisCompleteRuntimeException("调用目标异常！");
        } catch (NoSuchMethodException e) {
            throw new MybatisCompleteRuntimeException("实体类" + clazz.getName() + "不存在无参构造方法！");
        }
        return (T)obj;
    }
}
